/**
 * Base16 Encode Tool Component
 *
 * Dedicated tool for encoding text to hexadecimal with input encoding options.
 */
...
const TOOL_ID = 'base16-encode'

/**
 * Base16 Encode options component
 */
function Base16EncodeOptions({
  options,
  onOptionChange,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 p-3 rounded-md">
        <p>
          <strong>Encode:</strong> Convert text to hexadecimal (Base16). Output is lowercase hex.
 ...
 * Base16 Encode Tool Component
 */
export function Base16EncodeTool(): React.ReactElement {
  const {
    currentInput,
    currentOptions,
    setCurrentInput,
  ...
  useEffect(() => {
    setHeaderTitle('Base16 Encode')
    setHeaderDescription('Convert text to hexadecimal encoding')
  ...
  return (
    <div className="grid gap-4 h-full min-w-0 p-4" style={{ gridTemplateColumns: '1fr 2fr 2fr' }}>
      ...
      <div className="flex flex-col min-w-0 min-h-0 overflow-y-auto">
        <ToolOptions
          hasOptions={true}
          optionsComponent={Base16EncodeOptions}
