/**
 * Base32 Decode Tool Component
 *
 * Dedicated tool for decoding Base32 to text with case-insensitive and whitespace-tolerant input.
 */
...
const TOOL_ID = 'base32-decode'

function Base32DecodeOptions({
  options,
  onOptionChange,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 p-3 rounded-md">
        <p>
          <strong>Decode:</strong> Convert RFC 4648 Base32 back to text. Case-insensitive,
          whitespace-tolerant, padding-optional. Commonly used for TOTP/2FA secrets.
        </p>
      </div>
    </div>
  )
...
export function Base32DecodeTool(): React.ReactElement {
  const {
    currentInput,
    currentOptions,
    setCurrentInput,
  ...
  useEffect(() => {
    setHeaderTitle('Base32 Decode')
    setHeaderDescription('Convert Base32 to text')
  ...
  return (
    <div className="grid gap-4 h-full min-w-0 p-4" style={{ gridTemplateColumns: '1fr 2fr 2fr' }}>
      <div className="flex flex-col min-w-0 min-h-0 overflow-y-auto">
        <ToolOptions
          hasOptions={true}
          optionsComponent={Base32DecodeOptions}
