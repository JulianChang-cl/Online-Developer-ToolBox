...
/**
 * URL Encode options component (minimal - no special options needed)
 */
function URLEncodeOptions({
  options: _options,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <label
          htmlFor="url-encode-auto-update"
      ...
      <div className="text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 p-3 rounded-md">
        <p>
          <strong>URL Encode:</strong> Converts text to percent-encoded format for use in URLs.
          Follows RFC 3986 standard.
        </p>
        <p className="mt-2">
          <strong>Unreserved (not encoded):</strong> A-Z a-z 0-9 - . _ ~
        </p>
        <p className="mt-2">
          <strong>Encoded:</strong> Spaces become %20, special chars like = & ? become %XX sequences
        </p>
        <p className="mt-2">
          <strong>Note:</strong> Properly handles UTF-8 multi-byte characters (Chinese, emoji, etc.)
        </p>
      </div>
    </div>
  )
 ...
 * URL Encode Tool Component
 */
export function URLEncodeTool(): React.ReactElement {
  const {
    currentInput,
    currentOptions,
    setCurrentInput,
  ...
  useEffect(() => {
    setHeaderTitle('URL Encode')
    setHeaderDescription('Convert text to percent-encoded URL format')
  ...
  const handleOptionChange = (_key: string, _value: unknown): void => {
    // No options to handle for URL encode
  }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      try {
        const encoded = urlEncoderService.encode(localInput)
        setOutput(encoded)
      } catch (error) {
  ...
  return (
    <div className="grid gap-4 h-full min-w-0 p-4" style={{ gridTemplateColumns: '1fr 2fr 2fr' }}>
      ...
      <div className="flex flex-col min-w-0 min-h-0 overflow-y-auto">
        <ToolOptions
          hasOptions={true}
          optionsComponent={URLEncodeOptions}
      ...
      <div className="flex flex-col gap-3 min-w-0 min-h-0">
        <InputField
          ...
          placeholder="Enter text or URL parameters: hello world, key=value"
